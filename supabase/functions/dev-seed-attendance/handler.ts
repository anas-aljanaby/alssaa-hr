...
export type DevSeedUserClient = {
  auth: { getUser: () => Promise<{ data: { user: { id: string } | null }; error: unknown | null }> };
};

export type DevSeedDeps = {
  getEnv: () => DevSeedEnv;
  createUserClient: (authHeader: string) => DevSeedUserClient;
  createServiceClient: () => PunchServiceClient;
  /** Injected clock for deterministic tests. */
  now?: () => Date;
};

export async function handleDevSeedAttendance(req: Request, deps: DevSeedDeps): Promise<Response> {
  ...
  try {
    ...
    const profile = profileRaw as { org_id: string } | null;
    ...
    const offDays = policy?.weekly_off_days ?? [5, 6];
