-- Repair missing attendance automation objects in environments where migration
-- metadata says attendance migrations were applied but the schema drifted.
...
drop trigger if exists on_attendance_sessions_recalculate_daily_summary on public.attendance_sessions;
...
drop policy if exists "Users can read own attendance audit logs" on public.attendance_audit_log;
create policy "Users can read own attendance audit logs"
  on public.attendance_audit_log for select
  to authenticated
  using (employee_id = auth.uid());

drop policy if exists "Managers can read department attendance audit logs" on public.attendance_audit_log;
...
drop policy if exists "Admins can read all org attendance audit logs" on public.attendance_audit_log;
...
drop policy if exists "Service role can insert attendance audit logs" on public.attendance_audit_log;
...
drop trigger if exists protect_attendance_audit_log_update on public.attendance_audit_log;
...
drop trigger if exists protect_attendance_audit_log_delete on public.attendance_audit_log;
...
drop trigger if exists on_attendance_sessions_audit on public.attendance_sessions;
...
drop policy if exists "Users can insert own correction requests" on public.attendance_correction_requests;
...
drop policy if exists "Managers can read department correction requests" on public.attendance_correction_requests;
...
drop policy if exists "Managers can review department correction requests" on public.attendance_correction_requests;
...
drop policy if exists "Admins can read all org correction requests" on public.attendance_correction_requests;
...
drop policy if exists "Admins can review all org correction requests" on public.attendance_correction_requests;
...
drop trigger if exists enforce_org_attendance_correction_requests on public.attendance_correction_requests;
...
drop trigger if exists enforce_org_attendance_audit_log on public.attendance_audit_log;
