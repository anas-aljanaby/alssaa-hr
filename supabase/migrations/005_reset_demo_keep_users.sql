-- ============================================================
-- ALSSAA HR — Reset + Reseed Demo (Keep Users)
-- ------------------------------------------------------------
-- This script:
--   1) Clears all demo org activity data (attendance, leave, notifications, audit)
--   2) Resets demo leave balances and demo org config
--   3) Recreates demo departments, policy, seed data (same as 004_seed_demo.sql)
-- WITHOUT deleting auth.users rows, so existing demo sessions stay valid.
--
-- Effectively: "003_reset_demo_data.sql" (but keep users) + "004_seed_demo.sql"
-- in a single script.
-- ============================================================
...
begin
  -- --------------------------------------------------------
  -- 0. CLEAR EXISTING DEMO ORG DATA (BUT KEEP AUTH USERS)
  -- --------------------------------------------------------
  ...
  delete from public.audit_logs      where org_id = _demo_org;

  -- Reset leave balances for demo users to clean baseline
  ...
  -- Drop demo departments and attendance policy so we can recreate them
  delete from public.departments where org_id = _demo_org;
  delete from public.attendance_policy where org_id = _demo_org;

  -- Clear demo org general manager; seeding below will set it again
  update public.organizations
  set general_manager_id = null
  where id = _demo_org;

  -- --------------------------------------------------------
  -- 1. Hash the shared demo password once
  -- --------------------------------------------------------
  _pw_hash := extensions.crypt('DemoPass123!', extensions.gen_salt('bf'));

  -- --------------------------------------------------------
  -- 2. Attendance policy for demo org
  -- --------------------------------------------------------
  ...
  -- --------------------------------------------------------
  -- 4. Helper: insert/upsert one demo auth user + identity + profile
  -- --------------------------------------------------------
  ...
  -- --------------------------------------------------------
  -- 5. Create / refresh demo auth users, profiles, leave_balances
  -- --------------------------------------------------------
  ...
  -- --------------------------------------------------------
  -- 6. Set department managers and demo org general manager
  -- --------------------------------------------------------
  ...
  -- --------------------------------------------------------
  -- 7. Attendance logs — last 30 days, reproducible random
  -- --------------------------------------------------------
  ...
  -- --------------------------------------------------------
  -- 9. Update leave balances with realistic usage
  -- --------------------------------------------------------
  ...
  raise notice 'Demo reset+seed complete (users kept): 18 users, 4 departments, attendance, requests, notifications.';
end;
$$;
