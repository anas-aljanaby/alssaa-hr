...
import { getRequestTypeAr, getStatusAr, type RequestType } from '../../data/mockData';
import {
  Plus,
  X,
  Calendar,
...
export function RequestsPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    submitRequest({
      ...
      type: formData.type,
      fromDateTime: `${formData.fromDate}T${formData.fromTime}:00`,
      toDateTime: `${formData.toDate}T${formData.toTime}:00`,
      note: formData.note,
    });
    setShowForm(false);
    setFormData({
      ...
      {userRequests.length === 0 ? (
        ...
        <div className="space-y-3">
          {userRequests.map(req => (
            <div key={req.id} className={`bg-white rounded-xl p-4 border border-gray-100 border-r-4 shadow-sm ${statusBg(req.status)}`}>
